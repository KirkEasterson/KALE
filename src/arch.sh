...
#It's not officially part of KALE, but I use it for testing.
...
dialog --no-cancel --inputbox "Enter partitionsize in gb, separated by space (root & swap)." 10 60 2>psize
...
rm tz.tmp
curl https://raw.githubusercontent.com/kirkeasterson/KALE/master/src/chroot.sh > /mnt/chroot.sh && arch-chroot /mnt bash chroot.sh && rm /mnt/chroot.sh
